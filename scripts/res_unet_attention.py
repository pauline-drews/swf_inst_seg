"""
This is a model script for a Residual U-Net with attention. 

This code was taken and only slightly modified (parts deleted) from 
https://github.com/bnsreenu/python_for_microscopists/blob/master/224_225_226_models.py. 
(Author: Dr. Sreenivas Bhattiprolu)

Dr. Bhattiprolu has taken and heavily modified from:
https://github.com/MoleImg/Attention_UNet/blob/master/AttResUNet.py

Paper on attention U-net:
https://arxiv.org/pdf/1804.03999.pdf

Paper on (recurrent) residual Unet (R2U-Net):
...
def conv_block(x, filter_size, size, dropout, batch_norm=False):
    ...
    conv = layers.Conv2D(
        size, (filter_size, filter_size), padding="same")(conv)
    if batch_norm is True:
        conv = layers.BatchNormalization(axis=3)(conv)
    conv = layers.Activation("relu")(conv)
...
def repeat_elem(tensor, rep):
    ...
    #(None, 256,256,6), if specified axis=3 and rep=2.
     ...
     return layers.Lambda(lambda x, 
                          repnum: K.repeat_elements(x, repnum, axis=3),
...
def gating_signal(input, out_size, batch_norm=False):
    """
    resize the down layer feature map into the same dimension as the up layer 
    feature map using 1x1 conv
    :return: the gating feature map with the same dimension of the up layer 
    feature map
    """
...
# Getting the x signal to the same shape as the gating signal
    theta_x = layers.Conv2D(
        inter_shape, (2, 2), strides=(2, 2), padding='same')(x)  # 16
...
# Getting the gating signal to the same number of filters as the inter_shape
    ...
    upsample_g = layers.Conv2DTranspose(
        inter_shape, (3, 3),
        strides=(
            shape_theta_x[1] // shape_g[1], shape_theta_x[2] // shape_g[2]),
    ...
    upsample_psi = layers.UpSampling2D(
        size=(shape_x[1] // shape_sigmoid[1], 
              shape_x[2] // shape_sigmoid[2]))(sigmoid_xg)  # 32
