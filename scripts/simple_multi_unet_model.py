"""
This is a model script for a simple U-Net for multiclass case.

This code was taken and slightly modified (parts removed, metric changed) from 
...
def multi_unet_model(
        n_classes=4, IMG_HEIGHT=256, IMG_WIDTH=256, IMG_CHANNELS=1): # added n_classes
#Build the model
    ...
    # not one output, but n_classes outputs, and softmax instead of sigmoid
    outputs = Conv2D(n_classes, (1, 1), activation='softmax')(c9)
     
    model = Model(inputs=[inputs], outputs=[outputs])
    
    metr = tf.keras.metrics.MeanIoU(num_classes=n_classes)
    loss = tf.keras.losses.CategoricalFocalCrossentropy()

    model.compile(optimizer='adam', loss=loss, metrics=[metr])
