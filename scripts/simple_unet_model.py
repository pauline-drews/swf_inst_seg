"""
This is a model script for a simple U-Net for binary case.
...
################################################################
def simple_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS): # inputs mean that I have to use 256x256 for predictions
#Build the model
    ...
    outputs = Conv2D(1, (1, 1), activation='sigmoid')(c9) # output is probability
     
    model = Model(inputs=[inputs], outputs=[outputs])
    model.compile(
        optimizer='adam', loss='binary_crossentropy', metrics=['BinaryIoU'])
    model.summary()
